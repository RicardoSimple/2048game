...
Manager.prototype.defaultStart = function() {
  ...
  this.bestScore = this.storage.getBestScore().bestScore;
  // 如果存在缓存则恢复
  if (state) {
...
Manager.prototype._render = function() {
  ...
  this.storage.setBestScore({bestScore: this.bestScore});
  this.render.render(this.grid, { score: this.score, status: this.status },this.bestScore);
};
